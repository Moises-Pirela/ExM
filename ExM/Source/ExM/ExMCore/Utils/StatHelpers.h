...
{
	float baseStrength = rsqrt(strengthStat * 100.0f / throwableMass);
	float finalThrowStrength = baseStrength * modifiers;
	return finalThrowStrength;
}

constexpr float CalculateMaxHealth(int strengthStat, int vitalityStat, float baseMaxHealth, float modifiers)
{
